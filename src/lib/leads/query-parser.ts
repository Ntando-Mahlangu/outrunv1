...
- placesQuery: a short, clean phrase combining the industry/business type and location — what Google Places' text search should run on. Drop qualifiers Places can't use. Correct obvious misspellings in the business type too, not just the location — "plubers" → "plumbers", "electrican" → "electrician", "resturant" → "restaurant". A directory's own search rarely tolerates typos, so resolve to the well-known word the misspelling is clearly reaching for rather than preserving it verbatim.
